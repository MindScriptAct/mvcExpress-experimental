...
package runTestMvcExpressLive.mvcCore {
...
public class MediatorMapLiveFlex extends MediatorMapLive {
	...
	override public function mediate(viewObject:Object):void {
		if ((viewObject is uiComponentClass) && !viewObject['initialized']) {
			...
			var mediatorClasses:Vector.<Class> = mediatorMapOrderRegistry[viewClass];
			if (mediatorClasses) {
	...
	/**
	 * If any mediator is mediating viewObject: it calls onRemove, automatically removes all handler functions listening for test from that mediator and deletes it.
