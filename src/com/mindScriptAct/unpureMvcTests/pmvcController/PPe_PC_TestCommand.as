 ...
 */
public class PPe_PC_TestCommand extends UnpureSimpleCommand {

	/**
	 * @param note an <code>INotification</code>
	 */
	override public function execute(note:UnpureNotification):void {
		trace("PPe_PC_TestCommand execute :", note.getName(), note.getType(), note.getBody());
		var params:UnpureTestVo = note.getBody() as UnpureTestVo;

		AssertExpress.equals(note.getBody(), UnpureTestConst.PPe_PC, "Pmvc command got wrong data.");
		AssertExpress.equals(note.getName(), UnpureTestMessageTypes.PPe_PC_TEST, "Pmvc command got wrong type.");
